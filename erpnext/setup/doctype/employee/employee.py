...
class Employee(NestedSet):
    ...
    def validate_user_details(self):
        if self.user_id:
            data = frappe.db.get_value(
                "User", self.user_id, ["enabled", "user_image"], as_dict=1
            )
    ...
    def validate_status(self):
        if self.status == "Left":
            ...
            if reports_to:
                ...
                throw(
                    message, InactiveEmployeeStatusError, _("Cannot Relieve Employee")
                )
    ...
    def validate_for_enabled_user_id(self, enabled):
        ...
        if enabled == 0:
            frappe.throw(
                _("User {0} is disabled").format(self.user_id),
    ...
    def validate_preferred_email(self):
        if self.prefered_contact_email and not self.get(
            scrub(self.prefered_contact_email)
        ):
...
def validate_employee_role(doc, method=None, ignore_emp_check=False):
    ...
    if "Employee" in user_roles:
        frappe.msgprint(
            _("User {0}: Removed Employee role as there is no mapped employee.").format(
                doc.name
            )
        )
