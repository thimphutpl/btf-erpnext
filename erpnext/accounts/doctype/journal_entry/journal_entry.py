...
from erpnext.accounts.utils import get_account_type
...
class JournalEntry(AccountsController):
	...
	def autoname(self):
		prefix = frappe.db.get_value(
			"Journal Entry Series", self.naming_series, "prefix"
		)
		if not prefix:
			frappe.throw(
				"Please set prefix {}".format(
	...
	def validate_advance_accounts(self):
		...
		advance_accounts.add(
			frappe.get_cached_value(
				"Company", self.company, "default_advance_paid_account"
			)
		)
	...
	def update_advance_paid(self):
		...
		for d in self.get("accounts"):
			if d.is_advance:
				...
				elif d.reference_type=="Travel Authorization":
					frappe.db.sql("""
						UPDATE `tabTravel Authorization`
						
						SET advance_amount = %s
						WHERE name=%s
					""",(d.debit_in_account_currency,d.reference_name))
					frappe.db.commit()
	...
	def validate_inter_company_accounts(self):
		if (
			...
			doc = frappe.get_doc(
				"Journal Entry", self.inter_company_journal_entry_reference
			)
			account_currency = frappe.get_cached_value(
				"Company", self.company, "default_currency"
			)
			previous_account_currency = frappe.get_cached_value(
				"Company", doc.company, "default_currency"
			)
			if account_currency == previous_account_currency:
				if (
					self.total_credit != doc.total_debit
					or self.total_debit != doc.total_credit
				):
					frappe.throw(
						_(
	...
	def validate_stock_accounts(self):
		...
		for account in stock_accounts:
			...
			if account_bal == stock_bal:
				frappe.throw(
					_("Account: {0} can only be updated via Stock Transactions").format(
						account
					),
	...
	def apply_tax_withholding(self):
		...
		party_amount = sum(
			d.get(rev_debit_or_credit)
			for d in self.get("accounts")
			if d.account == party_account
		)
		...
		tax_withholding_details, advance_taxes, voucher_wise_amount = (
			get_party_tax_withholding_details(inv, self.tax_withholding_category)
		)
		...
		for d in self.get("accounts"):
			...
			if d.get("account") == party_account:
				d.update(
					{
						rev_debit_or_credit: party_amount
						- tax_withholding_details.get("tax_amount")
					}
				)
		...
		to_remove = [
			d
			for d in self.get("accounts")
			if not d.get(rev_debit_or_credit)
			and d.account == tax_withholding_details.get("account_head")
		]
			...
			if (
				...
				if asset.calculate_depreciation:
					...
					asset.db_set(
						...
						asset.value_after_depreciation - d.debit,
					)
	...
	def update_invoice_discounting(self):
		def _validate_invoice_discounting_status(
			inv_disc, id_status, expected_status, row_id
		):
			...
			if id_status != expected_status:
				frappe.throw(
					...
					).format(d.idx, expected_status, id_link)
				)
		...
		for inv_disc in invoice_discounting_list:
			...
			for d in self.accounts:
				if (
					d.account == inv_disc_doc.short_term_loan
					and d.reference_name == inv_disc
				):
					if self.docstatus == 1:
						if d.credit > 0:
	...
	def update_booked_depreciation(self, cancel=0):
		for d in self.get("accounts"):
			if (
				...
				and d.reference_name
				and frappe.get_cached_value("Account", d.account, "root_type")
				== "Expense"
				and d.debit
			):
				asset = frappe.get_doc("Asset", d.reference_name)
	...
	def unlink_advance_entry_reference(self):
		for d in self.get("accounts"):
			if d.is_advance == "Yes" and d.reference_type in (
				"Sales Invoice",
				"Purchase Invoice",
			):
			...
			if (
				...
				if asset.calculate_depreciation:
					...
					for fb_row in asset.get("finance_books"):
						...
						depr_schedule = get_depr_schedule(
							asset.name, "Active", fb_row.finance_book
						)
	...
	def unlink_inter_company_jv(self):
		if (
			...
			frappe.db.set_value(
				"Journal Entry", self.name, "inter_company_journal_entry_reference", ""
			)
	...
	def validate_cheque_info(self):
		if self.voucher_type in ["Bank Entry"]:
			if not self.cheque_no or not self.cheque_date:
				msgprint(
					_("Reference No & Reference Date is required for {0}").format(
						self.voucher_type
					),
					raise_exception=1,
				)
		...
		if self.cheque_date and not self.cheque_no:
			msgprint(
				_("Reference No is mandatory if you entered Reference Date"),
				raise_exception=1,
			)

	def validate_entries_for_advance(self):
		for d in self.get("accounts"):
			if d.reference_type not in (
				"Sales Invoice",
				"Purchase Invoice",
				"Journal Entry",
			):
				if (d.party_type == "Customer" and flt(d.credit) > 0) or (
					...
					if d.is_advance == "No":
						...
						d.reference_type in ("Sales Order", "Purchase Order")
						and d.is_advance != "Yes"
					):
						frappe.throw(
							_(
				...
				if d.is_advance == "Yes":
					if d.party_type == "Customer" and flt(d.debit) > 0:
						...
						frappe.throw(
							_("Row {0}: Advance against Supplier must be debit").format(
								d.idx
							)
						)
	...
	def validate_against_jv(self):
		for d in self.get("accounts"):
			if d.reference_type == "Journal Entry":
				account_root_type = frappe.get_cached_value(
					"Account", d.account, "root_type"
				)
				if (
		...
		for reference_name, total in self.reference_totals.items():
			...
			if reference_type in ("Sales Order", "Purchase Order"):
				...
				if cstr(order.status) == "Closed":
					frappe.throw(
						_("{0} {1} is closed").format(reference_type, reference_name)
					)
				...
				if account_currency == self.company_currency:
					...
					formatted_voucher_total = fmt_money(
						voucher_total,
						order.precision("base_grand_total"),
	...
	def validate_multi_currency(self):
		...
		for d in self.get("accounts"):
			...
			if (
				d.account_currency != self.company_currency
				and d.account_currency not in alternate_currency
			):
	...
	def create_remarks(self):
		...
		if self.cheque_no:
			if self.cheque_date:
				r.append(
					_("Reference #{0} dated {1}").format(
						self.cheque_no, formatdate(self.cheque_date)
					)
				)
			else:
				msgprint(
					_("Please enter Reference date"),
			...
			if d.reference_type == "Sales Invoice" and d.credit:
				r.append(
					_("{0} against Sales Invoice {1}").format(
						fmt_money(flt(d.credit), currency=self.company_currency),
						d.reference_name,
					)
				)
				...
				if (
					...
					r.append(
						_("{0} against Bill {1} dated {2}").format(
							...
							bill_no[0][0],
							bill_no[0][1]
							and formatdate(bill_no[0][1].strftime("%Y-%m-%d")),
						)
					)
	...
	def set_print_format_fields(self):
		...
		currency = bank_account_currency = party_account_currency = pay_to_recd_from = (
			None
		)
		party_type = None
		for d in self.get("accounts"):
			if d.party_type in ["Customer", "Supplier"] and d.party:
				...
				if pay_to_recd_from and pay_to_recd_from == d.party:
					party_amount += flt(d.debit_in_account_currency) or flt(
						d.credit_in_account_currency
					)
					party_account_currency = d.account_currency

			elif frappe.get_cached_value("Account", d.account, "account_type") in [
				"Bank",
				"Cash",
			]:
				bank_amount += flt(d.debit_in_account_currency) or flt(
					d.credit_in_account_currency
				)
			...
			for d in self.get("accounts"):
				if d.debit or d.credit or (self.voucher_type == "Exchange Gain Or Loss"):
					...
					for acc in with_tax:
						...
						tax_account_name = d.tax_account if (acc == d.tax_account) else None
				...
				# )
						gl_map.append(
								self.get_gl_dict(
									{
										"account": d.account if not tax_account_name else tax_account_name,
										"party_type": d.party_type,
										"due_date": self.due_date,
										"party": d.party,
										"against": d.against_account,
	...
	def get_balance(self, difference_account=None):
		if not self.get("accounts"):
			...
			if diff:
				...
				for d in self.get("accounts"):
					if (
						not d.credit_in_account_currency
						and not d.debit_in_account_currency
						and diff != 0
					):
						blank_row = d
	...
	def get_outstanding_invoices(self):
		...
		for d in self.get_values():
			...
			if self.write_off_based_on == "Accounts Receivable":
				...
				jd1.debit_in_account_currency = flt(
					d.outstanding_amount, self.precision("debit", "accounts")
				)
	...
	def get_values(self):
		cond = (
			f" and outstanding_amount <= {self.write_off_amount}"
		...
		if self.write_off_based_on == "Accounts Receivable":
			return frappe.db.sql(
				...
				and outstanding_amount > 0 {}""".format(
					"%s", cond
				),
				self.company,
				as_dict=True,
	...
	def validate_credit_debit_note(self):
		if self.stock_entry:
			if frappe.db.get_value("Stock Entry", self.stock_entry, "docstatus") != 1:
				frappe.throw(
					_("Stock Entry {0} is not submitted").format(self.stock_entry)
				)
			...
			if frappe.db.exists(
				...
				frappe.msgprint(
					...
					).format(self.voucher_type, self.name, self.stock_entry)
				)
...
def get_payment_entry_against_invoice(
	...
	if dt == "Sales Invoice":
		party_type = "Customer"
		party_account = (
			get_party_account_based_on_invoice_discounting(dn) or ref_doc.debit_to
		)
	else:
		party_type = "Supplier"
...
def get_payment_entry(ref_doc, args):
	...
	party_row = je.append(
		"accounts",
		{
			...
			"account_type": frappe.get_cached_value(
				"Account", args.get("party_account"), "account_type"
			),
	...
	if party_row.account_currency != ref_doc.company_currency or (
		bank_row.account_currency
		and bank_row.account_currency != ref_doc.company_currency
	):
		je.multi_currency = 1
...
def get_outstanding(args):
	...
	if args.get("doctype") == "Journal Entry":
		...
		party_type = (
			"Customer" if args.get("doctype") == "Sales Invoice" else "Supplier"
		)
		invoice = frappe.db.get_value(
			args["doctype"],
			args["docname"],
		...
		exchange_rate = (
			...
			if (args.get("account_currency") != company_currency)
			else 1
		)
...
def get_exchange_rate(
	...
	account_details = frappe.get_cached_value(
		"Account",
		account,
		["account_type", "root_type", "account_currency", "company"],
		as_dict=1,
	)
	...
	if account_currency != company_currency:
		if reference_type in ("Sales Invoice", "Purchase Invoice") and reference_name:
			exchange_rate = frappe.db.get_value(
				reference_type, reference_name, "conversion_rate"
			)

		# The date used to retreive the exchange rate here is the date passed
		# in as an argument to this function.
		elif (
			(not exchange_rate or flt(exchange_rate) == 1)
			and account_currency
			and posting_date
		):
			exchange_rate = get_exchange_rate(
				company_currency,account_currency,posting_date
			)
...
def get_average_exchange_rate(account):
	...
	if bank_balance_in_account_currency:
		...
		exchange_rate = (
			bank_balance_in_company_currency / bank_balance_in_account_currency
		)
...
def make_bank_payment(source_name, target_doc=None):
	def set_missing_values(obj, target, source_parent):
		...
		target.to_date = None
		target.paid_from = frappe.db.get_value(
			"Branch", target.branch, "expense_bank_account"
		)
		target.get_entries()
